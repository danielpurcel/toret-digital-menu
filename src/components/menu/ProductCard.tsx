...
export const ProductCard = ({ product, onOpen, badge }: Props) => {
  ...
  {product.bottlePrice ? (
    <div className="flex items-center gap-2">
              <span className="flex items-center gap-1">{formatPrice(product.price)}<svg className="h-3.5 w-3.5 text-toret-gold-warm" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M7 3c0 5 2 7 3 11h4c1-4 3-6 3-11Z"/><line x1="11" y1="14" x2="11" y2="19"/><line x1="7" y1="19" x2="15" y2="19"/></svg></span>
              <span className="h-3 w-px bg-toret-ink-muted/30"></span>
              <span className="flex items-center gap-1">{formatPrice(product.bottlePrice)}<svg className="h-3.5 w-3.5 text-toret-gold-warm" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M10 2h4v2c0 1 2 1.5 2 4v10.5a1.5 1.5 0 0 1-1.5 1.5h-5A1.5 1.5 0 0 1 8 18.5V8c0-2.5 2-3 2-4V2Z"/></svg></span>
            </div>
  ) : product.largePrice ? (
    <div className="flex items-center gap-1.5">
      <span className="flex items-center gap-0.5">{formatPrice(product.price)}<span className="text-[10px] text-toret-ink-muted font-normal">piccolo</span></span>
      <span className="h-3 w-px bg-toret-ink-muted/30"></span>
      <span className="flex items-center gap-0.5">{formatPrice(product.largePrice)}<span className="text-[10px] text-toret-ink-muted font-normal">grande</span></span>
    </div>
  ) : (
